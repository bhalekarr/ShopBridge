...
namespace ShopBridge.Controllers
{
    public class RoleController : Controller
    {
        ...
        public async Task<IActionResult> CreateRole(CreateRoleViewModel model)
        {
            ...
            {
                string[] roleNames = { "Product Admin" };
                ...
                foreach (var roleName in roleNames)
                {
                    ...
                    if (!roleExists)
                    {
                        ...
                        var emailCheck = await userManager.FindByEmailAsync(sa.Email);
                        if (emailCheck == null)
                        {
                            var saUser = await userManager.CreateAsync(sa, "Abcd@1234");
                            if (saUser.Succeeded)
                            {
                                await userManager.AddToRoleAsync(sa, "Product Admin");
                                return RedirectToAction("ListRoles", "Role");
                            }

                            foreach (IdentityError error in saUser.Errors)
                            {
